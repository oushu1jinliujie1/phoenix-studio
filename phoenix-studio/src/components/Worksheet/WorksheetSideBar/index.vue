        ...
        <x-spin :spinning="tabsLoading">
          <x-tabs v-model:active-key="activeTabKey">
            ...
            <template #tabBarExtraContent>
              <a-dropdown>
                <x-button data-test-id="oushudb-worksheet-resource-table-view-function-extra-dropdown" icon-name="worksheet/ellipsis"
                          type="text"/>
                <template #overlay>
                  <x-menu>
...
.v-oushudb-work-sheet-sidebar {
  ...
  .work-sheet-sidebar-title {
    ...
    display: flex;
    // 为什么加上这一行，好像多此一举？
    // 因为：sidebar tabs 宽度塌陷问题（当选择schema 的 table view function 数量为 0 时 触发）
    //width: 461px;
    // 这样可以自适应，但是会有塌陷问题
    width: 100%;
    @include font-normal();
